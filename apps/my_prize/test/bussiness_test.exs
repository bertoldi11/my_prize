...
d
  ...
  describe "new_prize/1" do
    test "new_prize creates a new prize with valid attributes" do
      ...
      {:ok, account} = Bussiness.new_account(%{"email" => "email@email.com.br", "name" => "Test User"})
    ...
    test "new_prize returns error if prize with name already exists for the account owner" do
      ...
      assert {:error, "Prize with this name already exists for the account owner"} = Bussiness.new_prize(attrs)
    end
  end

  describe "apply_for_prize/2" do
    test "apply_for_prize allows an account to apply for a prize" do
      {:ok, owner} = Bussiness.new_account(%{"email" => "owner@email.com.br", "name" => "Owner User"})
      {:ok, account} = Bussiness.new_account(%{"email" => "email@email.com.br", "name" => "Test User"})
      {:ok, prize} = Bussiness.new_prize(%{"name" => "Test Prize", "expiration_date" => Date.utc_today() |> Date.add(10), "account_owner_id" => owner.id})
    ...
    test "apply_for_prize returns error if prize does not exist" do
      {:ok, account} = Bussiness.new_account(%{"email" => "email@email.com.br", "name" => "Test User"})
      assert {:error, "Prize not found"} = Bussiness.apply_for_prize(account.id, "non_existent_prize_id")
    end
    test "apply_for_prize returns error if account has already applied for the prize" do
      ...
      assert {:error, "Account has already applied for this prize"} = Bussiness.apply_for_prize(account.id, prize.id)
    end
  end
  ...
  describe "raffle_prize/1" do
    test "raffle_prize raffles a prize and assigns a winner" do
      {:ok, account} = Bussiness.new_account(%{"email" => "email@email.com.br", "name" => "Test User"})
      {:ok, prize} = Bussiness.new_prize(%{"name" => "Test Prize", "expiration_date" => Date.utc_today() |> Date.add(10)})
      {:ok, _} = Bussiness.apply_for_prize(account.id, prize.id)
